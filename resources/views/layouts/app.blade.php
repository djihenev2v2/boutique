...
<head>
    ...
    <title>@yield('title', 'Accueil') — {{ $shopName }}</title>
...
<body class="bg-[#f8f9fb] min-h-screen font-sans antialiased">
    ...
    <aside id="sidebar" class="fixed top-0 left-0 z-50 h-screen w-[260px] bg-white/80 backdrop-blur-xl rounded-r-3xl flex flex-col -translate-x-full lg:translate-x-0 transition-transform duration-200 ease-out shadow-[0px_20px_40px_rgba(24,57,110,0.06)]">
        ...
        <div class="h-[68px] flex items-center gap-3 px-5 border-b border-[#edeef0]">
            ...
            <div>
                <p class="text-[#18396e] font-bold text-[15px] leading-tight tracking-tight">{{ $shopName }}</p>
        ...
        <nav class="flex-1 overflow-y-auto py-5 px-3 space-y-0.5">
            ...
            <a href="{{ route('cart.index') }}"
               class="group flex items-center gap-3 px-4 py-2.5 rounded-full text-[13px] font-medium transition-all duration-150 {{ request()->routeIs('cart.*', 'checkout*', 'checkout.confirmation') ? 'bg-[#002352] text-white shadow-md' : 'text-[#5d5f5f] hover:bg-[#f2f4f6] hover:text-[#18396e]' }}">
                <svg class="w-[18px] h-[18px] flex-shrink-0 {{ request()->routeIs('cart.*', 'checkout*', 'checkout.confirmation') ? 'text-white' : 'text-[#747780] group-hover:text-[#18396e]' }}" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
                ...
                <span data-cart-badge class="ml-auto text-[10px] font-bold min-w-[18px] h-[18px] px-1 rounded-full flex items-center justify-center {{ request()->routeIs('cart.*', 'checkout*', 'checkout.confirmation') ? 'bg-white text-[#002352]' : 'bg-[#002352] text-white' }}" style="display:{{ $cartCount > 0 ? 'flex' : 'none' }}">{{ $cartCount }}</span>
            </a>

            <a href="{{ route('order.tracking') }}"
               class="group flex items-center gap-3 px-4 py-2.5 rounded-full text-[13px] font-medium transition-all duration-150 {{ request()->routeIs('order.tracking*') ? 'bg-[#002352] text-white shadow-md' : 'text-[#5d5f5f] hover:bg-[#f2f4f6] hover:text-[#18396e]' }}">
                <svg class="w-[18px] h-[18px] flex-shrink-0 {{ request()->routeIs('order.tracking*') ? 'text-white' : 'text-[#747780] group-hover:text-[#18396e]' }}" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007z"/>
                </svg>
                <span>Suivi commandes</span>
            </a>

            <a href="{{ route('client.promo') }}"
               class="group flex items-center gap-3 px-4 py-2.5 rounded-full text-[13px] font-medium transition-all duration-150 {{ request()->routeIs('client.promo') ? 'bg-[#002352] text-white shadow-md' : 'text-[#5d5f5f] hover:bg-[#f2f4f6] hover:text-[#18396e]' }}">
                <svg class="w-[18px] h-[18px] flex-shrink-0 {{ request()->routeIs('client.promo') ? 'text-white' : 'text-[#747780] group-hover:text-[#18396e]' }}" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
            ...
            <a href="{{ route('cgv') }}"
               class="group flex items-center gap-3 px-4 py-2.5 rounded-full text-[13px] font-medium transition-all duration-150 {{ request()->routeIs('cgv') ? 'bg-[#002352] text-white shadow-md' : 'text-[#5d5f5f] hover:bg-[#f2f4f6] hover:text-[#18396e]' }}">
                <svg class="w-[18px] h-[18px] flex-shrink-0 {{ request()->routeIs('cgv') ? 'text-white' : 'text-[#747780] group-hover:text-[#18396e]' }}" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
        ...
        @else
        <div class="px-3 pb-4">
            <a href="{{ route('login') }}" class="flex items-center justify-center gap-2 bg-[#002352] text-white text-[13px] font-semibold rounded-2xl px-3 py-3 hover:bg-[#18396e] transition-colors">
                <svg class="w-[17px] h-[17px]" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
    ...
    <div class="lg:ml-[260px] min-h-screen flex flex-col">
        ...
        <header class="h-[68px] bg-transparent flex items-center justify-between px-4 lg:px-12 sticky top-0 z-30">
            ...
            <div class="flex items-center gap-2.5">
                ...
                @else
                <a href="{{ route('login') }}" class="flex items-center gap-1.5 text-[13px] font-semibold text-[#002352] hover:text-[#18396e] transition-colors">
